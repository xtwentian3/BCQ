...
class RegularActor(nn.Module):
    ...
    def log_pis(self, state, action=None, raw_action=None):
        ...
        log_std_a = self.log_std(a)
        std_a = torch.exp(log_std_a)
        normal_dist = td.Normal(loc=mean_a, scale=std_a, validate_args=True)  # 取正态分布
...
class BEAR(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005):
        for it in range(iterations):
            state_np, action, next_state_np, reward, done = self.sample_data(replay_buffer,batch_size)
    ...
    def sample_data(self,data, batch_size):
        ...
        # return (
        #     torch.FloatTensor(data['observations'][ind]).to(device),
        #     torch.FloatTensor(data['actions'][ind]).to(device),
        #     torch.FloatTensor(data['next_observations'][ind]).to(device),
        #     torch.FloatTensor(data['rewards'][ind]).to(device),
        #     torch.FloatTensor(~data['terminals'][ind]).to(device)
        # )
        return (
...
class BEAR_IS(object):
    ...
    def __init__(self, num_qs, state_dim, action_dim, max_action, delta_conf=0.1, use_bootstrap=True, version=0,
                 ...
                 mode='hardcoded', num_samples_match=10, mmd_sigma=10.0, lagrange_thresh=10.0, use_kl=False,
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005):
        for it in range(iterations):
            ...
            state_np, next_state_np, action, reward, done, mask, data_mean, data_cov = replay_buffer.sample(batch_size,
                                                                                                            with_data_policy=True)
            state = torch.FloatTensor(state_np).to(device)
            action = torch.FloatTensor(action).to(device)
            next_state = torch.FloatTensor(next_state_np).to(device)
            reward = torch.FloatTensor(reward).to(device)
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.98, tau=0.005):
        for it in range(iterations):
            ...
            if it % 2 == 0:
                ...
                if self.mode == 'auto':
                    actor_loss = -self.critic.q1(state, actor_actions[:, 0, :]).mean() + self.temperature.exp() * (
                                actor_log_pis - vae_log_pis).mean()
                else:
                    actor_loss = -self.critic.q1(state, actor_actions[:, 0, :]).mean() + self.temperature * (
                                actor_log_pis - vae_log_pis).mean()
                ...
                if self.mode == 'auto':
                    temperature_loss = (
                                                   vae_log_pis - actor_log_pis + self.kl_div_thresh).mean() * self.temperature.exp()
